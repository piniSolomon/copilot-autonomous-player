...
test.describe('Landing Page', () => {
  ...
  test('Reaction Test card links to correct page', async ({ page }) => {
    const rtCard = page.locator('.game-card[href*="reaction-test"]');
    await expect(rtCard).toBeVisible();
    await expect(rtCard).toHaveAttribute('href', /reaction-test/);
  });

  test('Color Merge card links to correct page', async ({ page }) => {
    const cmCard = page.locator('.game-card[href*="color-merge"]');
    await expect(cmCard).toBeVisible();
    await expect(cmCard).toHaveAttribute('href', /color-merge/);
  });

  test('Snake card links to correct page', async ({ page }) => {
    const snCard = page.locator('.game-card[href*="snake"]');
    await expect(snCard).toBeVisible();
    await expect(snCard).toHaveAttribute('href', /snake/);
  });

  test('Typing Speed card links to correct page', async ({ page }) => {
    const tsCard = page.locator('.game-card[href*="typing-speed"]');
    await expect(tsCard).toBeVisible();
    await expect(tsCard).toHaveAttribute('href', /typing-speed/);
  });

  test('Tic Tac Toe card links to correct page', async ({ page }) => {
    const tttCard = page.locator('.game-card[href*="tic-tac-toe"]');
    await expect(tttCard).toBeVisible();
    await expect(tttCard).toHaveAttribute('href', /tic-tac-toe/);
  });

  test('Brick Breaker card links to correct page', async ({ page }) => {
